export function generateDonateButtons(options) {
  let content='';
  let icon;
  let name;
  ...
  const downArrow = '<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><path fill="currentColor" d="m11.998 17l7-8h-14z"/></svg>'

  
  if (options.payments) {
    options.payments.forEach(payment => {
      if(!payment.url) return;
      const verticalMenu = options.orientation === 'vertical' || options.menuType === 'dropdown'
      let orientationCss = verticalMenu ? 'margin:0;padding:0;float:none;display:flex;flex-direction:column;align-self:stretch' : '';
      let iconColor = options.iconColor ? `color: ${options.iconColor};` : 'color: #333';
      let textColor = options.textColor ? `color: ${options.textColor};` : 'color: #333';
      
      let justifyContent = 'start';
      let btnStyle = `float:none;margin:0;padding:0;align-items: center;border:1px solid #ccc; padding: 4px 10px;display:flex;align-content: center; justify-content: ${justifyContent};${iconColor};${options.btnStyle}`
      const iconMatch = Object.keys(iconPack).find(key=> payment.url.includes(key))
      let icon = iconMatch && !options.hideIcon ? (iconPack[iconMatch] || payment?.icon) : '';
      let paymentName = options.hideName ? '' : `<span style='margin:0;padding:0;${textColor}'>${payment.text || payment.name || iconMatch}</span>`
      content += `<a target=_blank style='gap:1rem;${btnStyle}' class='${options.btnClass}' href="${payment.url}">${icon} ${paymentName}</a>`;
    })
    ...
    if (options.menuType === 'dropdown') {
      ...
      menu.innerHTML = `<div style='width: ${options.menuWidth};cursor: pointer;'><div style='display:flex;align-items:center;gap:4px;padding:0px 10px'>${loveIcon} ${options.menuText} ${downArrow}</div></div>`
      ...
      let textColor = options.textColor ? `color: ${options.textColor};` : 'color: #333';
      const verticalMenu = options.orientation === 'vertical' || options.menuType === 'dropdown'
      let orientationCss = verticalMenu ? 'margin:0;padding:0;float:none;display:flex;flex-direction:column;align-self:stretch' : 'display:flex;flex-direction:row;gap:5px';
      let div_start = `<div style='margin:0;padding:0;${orientationCss};${iconColor};${options.btnWrapperStyle}' title='Donate' class="btn-group" role="group">`;
      let div_end = '</div>';
      donateContainer.innerHTML = `${div_start}${content}${div_end}`
    }
  }
}